...
//		System.out.println("EL表达式： "+str);
    ...
    public static void getSlidePoint(TestStep step) throws Exception {
        String points = step.getSlidePoint();

        String[] strArr = points.split(",");
        if (strArr.length != 4) {
            throw new Exception("滑动坐标的数组应该为4个点");
        }
        int[] ponitArr = new int[strArr.length];
        for (int i = 0; i < strArr.length; ++i) {
            ponitArr[i] = Integer.valueOf(strArr[i]);
        }

        PointOption pointStrat = new PointOption();
        PointOption pointEnd = new PointOption();
        WaitOptions waitOptions = new WaitOptions();
        step.getAdriver().manage().timeouts().implicitlyWait(1000, TimeUnit.MILLISECONDS);
        TouchAction tAction = new TouchAction(step.getAdriver());
        pointStrat.withCoordinates(ponitArr[0], ponitArr[1]);
        pointEnd.withCoordinates(ponitArr[2], ponitArr[3]);
        tAction.press(pointStrat).waitAction(waitOptions.withDuration(Duration.ofSeconds(5)))
